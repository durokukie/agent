...
item claim, lease 갱신, 성공 확정, 실패 기록만 제공한다. Claim adapter가 bounded poison
batch를 처리했지만 아직 eligible row가 남을 수 있으면 `NotificationClaimBatchProgress`를
반환해 no-work와 구별한다.
...
eligible row를 계속 찾고 cap 도달은 progress로 반환해 다음 batch를 drain한다. 같은
...
sender를 취소하고 stale 결과를 확정하지 않는다. Renewal의 모든 예외는 sender task를
cancel하고 종료 결과를 회수하므로 shutdown 뒤 orphan task를 남기지 않는다. 외부
`notification_id` 멱등성은 crash와
