...
오류는 전부 {code, message} 객체다 (api-spec 공통 절).
"""
...
async def chat(
    ...
    _, conversation = _load(conversation_id, user, store)
    ...
    async with conversation.lock:
        ...
        if not created:
            ...
            return run.response_payload          # 재전송 — 실행하지 않고 저장된 응답

        try:
            payload, result = await _server._chat_turn(session, body.text)
        except HTTPException as exc:
            store.finish_run(run.id, status="failed", error=_wrap(exc).detail)
            raise _wrap(exc)
        except Exception as exc:                 # 모델·툴 예외 — run 은 실패로 남기고 세션은 유지
            logger.exception("run 실패 (conversation=%s, run=%s)", conversation_id, run.id)
            store.finish_run(run.id, status="failed", error={"code": "RUN_FAILED", "message": str(exc)})
            raise _error(500, "RUN_FAILED", f"요청 처리에 실패했다: {exc}") from exc
...
async def approve(
    ...
    async with conversation.lock:
        ...
        try:
            ...
            raise _wrap(exc)
        return _record_outcome(store, conversation, outcome, result, kind="approve",
