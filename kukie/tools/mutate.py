...
- LLM 작성 필드(intent/expected_effects/side_effects)는 MVP에서 제외.
  필요해지면 나중에 추가한다. Action Plan에는 코드가 아는 사실만 기록.
...
def apply_manifest(ctx: RunContext[Deps], manifest_yaml: str) -> KubectlResult:
...
def scale_resource(ctx: RunContext[Deps], kind: str, name: str, replicas: int,
                   namespace: str) -> KubectlResult:
    """리소스의 레플리카 수를 조정한다 (kubectl scale)."""
    raise NotImplementedError  # TODO

def rollout_restart(ctx: RunContext[Deps], kind: str, name: str,
                    namespace: str) -> KubectlResult:
    """Deployment 등을 재시작한다 (kubectl rollout restart)."""
    raise NotImplementedError  # TODO

def delete_resource(ctx: RunContext[Deps], kind: str, name: str,
                    namespace: str) -> KubectlResult:
