...
3. 현재 모드의 툴로 할 수 없지만 다른 모드에서 지원하는 작업은 해당 모드로 안내한다.
   어떤 모드에서도 제공된 툴로 지원하지 않는 작업은 실행하려 하지 말고,
   사용자가 직접 터미널에 실행할 kubectl 명령과 각 플래그의 의미를 안내한다.
...
모드별 역할:
- 학습: 개념 설명, 리소스 목록 및 스키마 조회
- 진단: 상세 상태, 이벤트, 로그를 통한 원인 분석
- 실습: 승인 절차를 거친 리소스 생성·수정·확장·재시작·삭제

현재 답변과 실행 결과를 바탕으로 권장하는 다음 행동을
suggested_next_action에 짧게 작성한다.
현재 모드에서 가능한 행동이면 모드 전환을 제안하지 않는다.
다른 모드의 기능이 필요하면 전환할 모드 이름과 이유를 함께 안내한다.
추가 행동이 필요하지 않으면 null로 반환한다.
제안은 모드 전환, 작업 실행 또는 사용자 승인을 의미하지 않는다."""
...
#     매개변수(narration, suggested_next_action)뿐이고, steps는 본문에서 코드가 실행 기록으로 채운다.
...
def add_current_mode(ctx: RunContext[Deps]) -> str:
    ...
    그래서 모델이 문맥에서 추론하게 두지 않고, 현재 모드와 "다른 모드의 과거 답변을 근거로 거절하지
    마라" 를 매 턴 직접 말한다.

    한정 둘을 빠뜨리면 안 된다 (자동 리뷰 지적).
      - "다른 모드의" — 빼면 학습·진단에서의 **정당한 거절**(BASE_PROMPT 3번, 앱의 [실습 모드로]
        버튼이 여기서 나온다)까지 눌러 버린다. #60 과 반대 방향의 같은 병이다
      - "클러스터에 쓸 수 있는" — allowed_tools 에는 응답을 마무리하는 출력 툴(build_KukieResponse 등)이
        없다. "이것뿐" 이라고 못 박으면 모델이 그 필수 툴을 안 부를 수 있다
    """
    skill = ctx.deps.skill
    tools = ", ".join(sorted(skill.allowed_tools)) or "없음"
    return (
        f"[현재 모드: {skill.name}] 지금 클러스터에 쓸 수 있는 툴은 {tools} 뿐이다 "
        "(응답을 마무리하는 출력 툴은 여기 해당하지 않는다 — 늘 그대로 호출한다).\n"
        "대화 기록에 **다른 모드에서** 한 답변(예: \"학습 모드에서는 실행할 수 없습니다\")이 남아 있어도 "
        "그것은 지나간 상태다. 지금 무엇이 가능한지는 위에 적힌 현재 모드와 지금 노출된 툴이 정한다. "
        "**다른 모드의 과거 답변을** 근거로 거절하지 마라. "
        "지금 모드에 없는 일을 요청받으면 예전처럼 거절하고 해당 모드를 안내한다 — 그건 올바른 답변이다."
    )
